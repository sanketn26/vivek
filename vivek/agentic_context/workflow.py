...
# Configure logging
def _configure_logging(log_level: str = "INFO", log_format: Optional[str] = None) -> logging.Logger:
...
def _validate_required_params(**kwargs) -> None:
    ...
    for param_name, value in kwargs.items():
        ...
        if isinstance(value, (list, tuple)) and len(value) == 0:
            raise ValidationError(f"Required parameter '{param_name}' cannot be empty list")
...
class TaskContext:
    ...
    def __init__(self, task_id: str, description: str, tags: List[str],
                 storage: ContextStorage, retriever, config: Dict[str, Any]):
        ...
        try:
            ...
            if not task_id.replace('_', '').replace('-', '').isalnum():
                logger.warning(f"Task ID '{task_id}' contains special characters, consider using alphanumeric only")

            # Validate description
            if len(description.strip()) < 10:
                logger.warning(f"Task description is very short ({len(description.strip())} chars): {description}")
            ...
            if len(tags) > 10:
                logger.warning(f"Large number of tags ({len(tags)}) for task, consider reducing")
            ...
            logger.error(f"Unexpected error during TaskContext validation: {str(e)}", exc_info=True)
        ...
        try:
            ...
            if context_data.get("activity"):
                ...
                if activity.get("description"):
                    ...
                    if activity.get("planner_analysis"):
                        parts.append(f"Planner Analysis:\n{activity['planner_analysis']}")
                    parts.append("")
            ...
            if include_history:
                try:
                        ...
                        for i, item in enumerate(relevant, 1):
                            try:
                                ...
                                if item.get('matched_tags'):
                                    parts.append(f"   Matched tags: {', '.join(item['matched_tags'])}")
                            except (KeyError, TypeError) as e:
                                logger.warning(f"Error processing retrieved item {i}: {str(e)}")
                                continue
                        parts.append("")
                    else:
                    ...
                    parts.append("=== RELEVANT CONTEXT FROM HISTORY ===")
                    parts.append("Unable to retrieve historical context due to an error.")
                    parts.append("")

            parts.append("=== INSTRUCTIONS ===")
            parts.append("Focus on the current task. Use relevant context to inform your implementation.")

            prompt = "\n".join(parts)
            logger.debug(f"Prompt built successfully for task: {self.task_id}, length: {len(prompt)}")
            return prompt

        except Exception as e:
            logger.error(f"Error building prompt for task {self.task_id}: {str(e)}", exc_info=True)
    ...
    def record_action(self, content: str, **metadata):
        ...
        try:
            ...
            logger.error(f"Validation error recording action for task {self.task_id}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Error recording action for task {self.task_id}: {str(e)}", exc_info=True)
    ...
    def record_decision(self, content: str, **metadata):
        ...
        try:
            ...
            logger.error(f"Validation error recording decision for task {self.task_id}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Error recording decision for task {self.task_id}: {str(e)}", exc_info=True)
    ...
    def record_learning(self, content: str, **metadata):
        ...
        try:
            ...
            logger.error(f"Validation error recording learning for task {self.task_id}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Error recording learning for task {self.task_id}: {str(e)}", exc_info=True)
    ...
    def set_result(self, result: str):
        ...
        try:
            ...
            logger.error(f"Error setting result for task {self.task_id}: {str(e)}", exc_info=True)
        ...
        try:
            if self._result:
                ...
                try:
                    ...
                    logger.error(f"Error completing task {self.task_id}: {str(e)}", exc_info=True)
                ...
                try:
                    ...
                    logger.error(f"Error adding result context for task {self.task_id}: {str(e)}", exc_info=True)

            logger.info(f"Task {self.task_id} finalized successfully")

        except Exception as e:
            logger.error(f"Unexpected error during task finalization for {self.task_id}: {str(e)}", exc_info=True)
...
class ActivityContext:
    ...
    def __init__(self, activity_id: str, description: str, tags: List[str],
                 mode: str, component: str, planner_analysis: str,
                 storage: ContextStorage, retriever, config: Dict[str, Any]):
        ...
        try:
            ...
            # Validate description
            if len(description.strip()) < 10:
                logger.warning(f"Activity description is very short ({len(description.strip())} chars): {description}")
            ...
            if len(planner_analysis.strip()) < 20:
                logger.warning(f"Planner analysis is very short ({len(planner_analysis.strip())} chars)")
            ...
            logger.error(f"Unexpected error during ActivityContext validation: {str(e)}", exc_info=True)
        ...
        logger.debug(f"ActivityContext initialized successfully for activity: {activity_id}")
    ...
    def task(self, description: str, tags: Optional[List[str]] = None, **metadata):
        ...
        try:
            ...
            logger.error(f"Task creation validation failed in activity {self.activity_id}: {str(e)}")
            raise
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to create task {task_id} in storage: {str(e)}", exc_info=True)
                raise StorageError(f"Failed to create task in storage: {str(e)}")

            # Create context object
            try:
                task_context = TaskContext(
                    task_id, description, task_tags,
                    self._storage, self._retriever, self._config
                )
                logger.debug(f"TaskContext object created: {task_id}")

            except Exception as e:
                logger.error(f"Failed to create TaskContext object for {task_id}: {str(e)}", exc_info=True)
                # Attempt to clean up the storage entry
                try:
                    # Note: We don't have a direct way to remove a task, so we just log
                    logger.warning(f"Task {task_id} was created in storage but TaskContext creation failed")
                except Exception as cleanup_error:
                    logger.error(f"Error during cleanup after TaskContext failure: {str(cleanup_error)}")
            ...
            logger.error(f"Error in task context manager for {task_id or 'unknown'}: {str(e)}", exc_info=True)
            raise
            ...
            if task_context is not None:
                try:
                    ...
                    logger.error(f"Error during task finalization for {task_id}: {str(e)}", exc_info=True)
                    # Don't raise exception during finalization to avoid masking original errors

            logger.info(f"Task context manager completed for activity {self.activity_id}")
...
class SessionContext:
    ...
    def __init__(self, session_id: str, original_ask: str,
                 high_level_plan: str, storage: ContextStorage,
                 retriever, config: Dict[str, Any]):
        ...
        try:
            ...
            if len(original_ask.strip()) < 10:
                logger.warning(f"Original ask is very short ({len(original_ask.strip())} chars): {original_ask}")

            # Validate high_level_plan
            if high_level_plan and len(high_level_plan.strip()) < 20:
                logger.warning(f"High level plan is very short ({len(high_level_plan.strip())} chars)")
            ...
            logger.error(f"Unexpected error during SessionContext validation: {str(e)}", exc_info=True)
        ...
        logger.debug(f"SessionContext initialized successfully for session: {session_id}")
    ...
    @contextmanager
    def activity(self, description: str, tags: List[str],
                 mode: str, component: str, planner_analysis: str,
                 **metadata):
        """
        ...
        try:
            ...
            logger.error(f"Activity creation validation failed in session {self.session_id}: {str(e)}")
            raise
        ...
        try:
            ...
            try:
                _safe_execute_operation(
                    ...
                    activity_id, description, tags, mode,
                    component, planner_analysis, **metadata
                )
                logger.debug(f"Activity created in storage: {activity_id}")

            except Exception as e:
                logger.error(f"Failed to create activity {activity_id} in storage: {str(e)}", exc_info=True)
            ...
            try:
                ...
                logger.error(f"Failed to record planner decision for activity {activity_id}: {str(e)}", exc_info=True)
            ...
            try:
                activity_context = ActivityContext(
                    activity_id, description, tags, mode,
                    component, planner_analysis,
                    self._storage, self._retriever, self._config
                )
                logger.debug(f"ActivityContext object created: {activity_id}")

            except Exception as e:
                logger.error(f"Failed to create ActivityContext object for {activity_id}: {str(e)}", exc_info=True)
                # Attempt to clean up the storage entry
                try:
                    logger.warning(f"Activity {activity_id} was created in storage but ActivityContext creation failed")
                except Exception as cleanup_error:
                    logger.error(f"Error during cleanup after ActivityContext failure: {str(cleanup_error)}")
            ...
            logger.error(f"Error in activity context manager for {activity_id or 'unknown'}: {str(e)}", exc_info=True)
            raise
            ...
            if activity_context is not None:
                try:
                    logger.debug(f"Activity context manager completed for: {activity_id}")
                    # Activity auto-completes - no explicit finalization needed
                except Exception as e:
                    logger.error(f"Error during activity cleanup for {activity_id}: {str(e)}", exc_info=True)

            logger.info(f"Activity context manager completed for session {self.session_id}")
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to initialize ContextStorage: {str(e)}", exc_info=True)
            ...
            logger.error(f"Unexpected error during ContextWorkflow initialization: {str(e)}", exc_info=True)
            raise ValidationError(f"ContextWorkflow initialization failed: {str(e)}")
    
    @contextmanager
    def session(self, original_ask: str, high_level_plan: Optional[str] = None,
                 **metadata):
        """
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to create session {session_id} in storage: {str(e)}", exc_info=True)
            ...
            try:
                session_context = SessionContext(
                    ...
                    self.storage, self.retriever, self.config
                )
                logger.debug(f"SessionContext object created: {session_id}")

            except Exception as e:
                logger.error(f"Failed to create SessionContext object for {session_id}: {str(e)}", exc_info=True)
                # Attempt to clean up the storage entry
                try:
                    logger.warning(f"Session {session_id} was created in storage but SessionContext creation failed")
                except Exception as cleanup_error:
                    logger.error(f"Error during cleanup after SessionContext failure: {str(cleanup_error)}")
            ...
            logger.error(f"Error in session context manager for {session_id or 'unknown'}: {str(e)}", exc_info=True)
            raise

        finally:
            # Session auto-completes - no explicit finalization needed
            logger.info(f"Session context manager completed for session {session_id or 'unknown'}")
    ...
    def switch_strategy(self, new_strategy: str):
        ...
        try:
            ...
            try:
                ...
                test_retriever = RetrieverFactory.create_retriever(self.storage, {"retrieval": {"strategy": new_strategy}})
                logger.debug(f"Strategy '{new_strategy}' validated successfully")

            except Exception as e:
                logger.error(f"Invalid retrieval strategy '{new_strategy}': {str(e)}", exc_info=True)
            ...
            try:
                ...
                logger.info(f"Successfully switched to retrieval strategy: {new_strategy}")
                print(f"✓ Switched to retrieval strategy: {new_strategy}")

            except Exception as e:
                logger.error(f"Failed to create retriever with strategy '{new_strategy}': {str(e)}", exc_info=True)
                raise RetrievalError(f"Failed to switch strategy: {str(e)}")

        except (ValidationError, RetrievalError):
            raise
        except Exception as e:
            logger.error(f"Unexpected error switching strategy to '{new_strategy}': {str(e)}", exc_info=True)
    ...
    def export(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error exporting context database: {str(e)}", exc_info=True)
                # Continue with empty context data
                context_data = {}

            export_data = {
                "sessions": sessions_data,
                "context_db": context_data
            }

            logger.info(f"Successfully exported workflow context ({len(sessions_data)} sessions, {len(context_data)} categories)")
            return export_data
