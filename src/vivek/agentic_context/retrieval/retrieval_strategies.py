...
class RetrievalCache:
    ...
    def _make_key(
        self, strategy: str, query_tags: List[str], query_desc: str, max_results: int
    ) -> str:
...
class RetrieverFactory:
    ...
    def create_retriever(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize {strategy} retriever: {e}") from e

    @staticmethod
    def create_retriever_with_config(
        context_storage: ContextStorage, retrieval_config: RetrievalConfig
    ) -> BaseRetriever:
        """
